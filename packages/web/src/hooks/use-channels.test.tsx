...
// The hook only talks to api.channels / api.createChannel over the network; the
...
function msg(channel: string, content: string, id: string, at = Date.now()): Message {
  return {
    id,
    participantId: "p2",
    authorName: "bot",
...
// Flush pending async state updates (the channel-list fetch resolves on a
...
describe("useChannels — unread + mention routing", () => {
  it("increments unread for a non-focused channel and ignores the focused one", async () => {
    localStorage.setItem("club_channel", "general");
    const { result } = renderHook(() => useChannels(conn, "alice"));
    await act(async () => {
      result.current.recordIncoming(msg("deploy-debug", "hi", "m1"));
      result.current.recordIncoming(msg("general", "current channel msg", "m2"));
    });
  ...
  it("fires a toast for a cross-channel @mention and flags the channel mention", async () => {
    localStorage.setItem("club_channel", "general");
    const { result } = renderHook(() => useChannels(conn, "alice"));
    await act(async () => {
    ...
    expect(result.current.toasts[0].channel).toBe("deploy-debug");
    expect(result.current.toasts[0].messageId).toBe("m1");
  });

  it("does not fire a toast for a mention in the focused channel", async () => {
    localStorage.setItem("club_channel", "general");
    const { result } = renderHook(() => useChannels(conn, "alice"));
    await act(async () => {
...
describe("useChannels — switching", () => {
  it("clears the target channel's unread + toasts and persists the choice", async () => {
    localStorage.setItem("club_channel", "general");
    const { result } = renderHook(() => useChannels(conn, "alice"));
    await act(async () => {
      result.current.recordIncoming(msg("deploy-debug", "hey @alice", "m1"));
    ...
    expect(result.current.toasts).toHaveLength(1);
    act(() => {
      result.current.switchChannel("deploy-debug");
    });
    expect(result.current.currentChannel).toBe("deploy-debug");
    expect(result.current.unread["deploy-debug"]).toEqual({ count: 0, mention: false });
    expect(result.current.toasts).toHaveLength(0);
    expect(localStorage.getItem("club_channel")).toBe("deploy-debug");
  });
});

describe("useChannels — sorting (unread-first, then most-recently-active)", () => {
  it("surfaces unread channels before read ones regardless of activity", async () => {
    localStorage.setItem("club_channel", "general");
    const { result } = renderHook(() => useChannels(conn, "alice"));
    await flush();
    expect(result.current.channels.length).toBe(2);
    // Both read: deploy-debug (newer activity 200) sorts before general (100).
    expect(result.current.sortedChannels.map((r) => r.slug)).toEqual(["deploy-debug", "general"]);
  ...
  it("an unread channel with older activity still sorts ahead of a read channel", async () => {
    localStorage.setItem("club_channel", "deploy-debug");
    const { result } = renderHook(() => useChannels(conn, "alice"));
    await flush();
    ...
    const slugs = result.current.sortedChannels.map((r) => r.slug);
