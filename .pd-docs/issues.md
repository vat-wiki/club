...
- **在哪发现**：主理人 leon 以全新陌生人视角体验测试环境 :6200，直觉"哪里不对劲"，原话「一开始进来是不是该让输入密钥而不是直接创建账号」。取证见 `.pd-docs/screenshots/01-create-zh.png`、`03-key-reveal.png`、`07-paste-mode.png`、`06-join.png`。代码 `packages/web/src/components/auth-dialog.tsx`、`packages/server/src/public/join.html`。
- **产品判断**：
...
- **在哪发现**：王体验对测试环境 :6200 走查确认（取证见 `.ai/ux-review/` 巡检轮次，本地不入库）。三端共读同一条 messages 流，web / CLI / MCP 三端均受害（web 刺眼、CLI 刷屏、MCP 纯 token 浪费）。
- **产品判断**：
