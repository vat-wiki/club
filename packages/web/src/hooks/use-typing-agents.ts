 ...
 *
 * Drives the live list of participants currently composing a reply (an agent
 * processing an @mention OR a human typing). The SDK parses the `agent_thinking`
 * / `agent_idle` SSE named events and dispatches them to the callbacks returned
 * here; wire those callbacks into `ClubClient.stream()`'s `StreamOptions`.
 *
 * Semantics:
 *   - `agent_thinking` -> add / refresh the participant in the set (id-keyed).
 *   - `agent_idle`     -> remove the participant from the set.
 *
 * The server guarantees an idle is always emitted on reply / error / offline,
 * and also runs a TTL reaper as a backstop, so the indicator can't get stuck on.
 ...
 *
 * @param selfId - Optional participant id of the current viewer. When provided,
 *                 that participant is always excluded from the `agents` array.
 * @returns `{ agents, onThinking, onIdle }` — `agents` is the current set of
 *                 typing participants; `onThinking` and `onIdle` are stable
 *                 callbacks to wire into the SSE `StreamOptions`.
 *
 * @example
 * const { agents, onThinking, onIdle } = useTypingAgents(me?.id);
 * useMessageStream(conn, { onAgentThinking: onThinking, onAgentIdle: onIdle });
 */
