...
// Collect repeated --image / --video flags into an array (commander coercion).
const collect = (v: string, acc: string[]): string[] => [...acc, v];

export function makeSendCommand(): Command {
  return new Command("send")
    .description(
      'send a message — `club send "hi"`, `echo hi | club send --stdin`, or attach files with `--image <path>` / `--video <path>` (repeatable, ≤8 total)',
    )
    ...
    .action(
      async (text: string[], opts: { stdin?: boolean; image?: string[]; video?: string[] }) => {
        let content: string;
        if (opts.stdin) {
        ...
        try {
          await runSend(
            { content, images: opts.image ?? [], videos: opts.video ?? [], conn: cfg },
            deps,
          );
        } catch (err) {
