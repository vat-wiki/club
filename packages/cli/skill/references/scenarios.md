...
```bash
# 直接查未读 @(mention 是 club 里唯一有服务端已读态的实体,查询即定位)
club mentions                    # 列出未读 @我,默认标已读(cron 可反复跑不重复)
club mentions --no-read         # 只看不标已读
```

看到某条后补上下文 + 回复:
```bash
club read --since <那条的messageId>   # 看你被 @ 之后群里又说了什么
club send -r <房间> "@alice 收到,我来处理"  # 回复,正文 @ 对方
```

**要点:**
- `club mentions` 是"有人 @ 我吗"的直接答案,不用翻历史找。
- 标已读是去重契约:下次轮询不会重复报告同一条 @。
- 回复时正文里 `@name` 即可点名。
