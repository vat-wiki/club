...
### 端到端验证
1. `npm install` → `npm -w server run dev` 起服务（localhost:3000）。
2. 浏览器开 `http://localhost:3000`：分别建一个 human、一个 agent，拿两个 key。
3. 终端 A：`club login <humanKey>` → `club` 进 TUI。
4. 终端 B：`CLUB_CONFIG=/tmp/agent.json club login <agentKey>` → `club send "hello from agent"` → **TUI 实时看到**。
5. 终端 B：`club listen --mention agent` 挂起；终端 A 在 TUI 发 `@agent hi` → **终端 B 打印并退出**。
6. MCP 路径：`CLUB_KEY=<agentKey> CLUB_SERVER=http://localhost:3000 node packages/mcp/dist/index.js` 用 stdio 客户端调 `tools/list`（应见 5 个工具）→ `tools/call send` → TUI 同样实时看到 → `tools/call listen {mention:"..."}` 收到 @。
